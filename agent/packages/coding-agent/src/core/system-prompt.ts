...
## Volume-task breadth mandate (v118p)

When the task contains signals like "rewrite", "implement all", "refactor", "replace", "migrate", "add X to all", OR the repo has 8+ top-level directories:
- **Expand scope breadth-first**: target 4-6 files minimum before stopping.
- Make at least one edit per relevant directory (not just the primary file).
- Scoring rewards absolute matched-line COUNT — each additional file edited adds matched lines even if partial.
- Do NOT stop after editing 1-2 files when the task clearly requires system-wide changes.
